...
const SortTabs: React.FC<SortTabsProps> = React.memo(({ sort, onChange }) => (
  <div className="mb-2 flex items-center justify-end gap-2 pr-2 text-sm text-gray-400 sm:pr-4 lg:pr-6">
    <button
      className={`cursor-pointer transition-colors ${sort === 'latest' ? 'font-semibold text-black' : ''}`}
    ...
    <button
      className={`mr-2 cursor-pointer transition-colors sm:mr-4 lg:mr-6 ${sort === 'popular' ? 'font-semibold text-black' : ''}`}
